...
Original file is located at
    https://colab.research.google.com/drive/15kdzZBygkkZeDK41LiINrmECcsASoX0t
...
def format_option_chain_data_into_dataframe(data):
    ...
    for item in data['data']:
        # Initialize general info dictionary
        general_info = {}
        for key in ['expiry', 'strike_price', 'underlying_key', 'underlying_spot_price', 'pcr']:
            if key in item:
                general_info[key] = item[key]
        general_data.append(general_info)

        # Extract call option data if available
        if 'call_options' in item and isinstance(item['call_options'], dict):
            ...
            if 'market_data' in call_option and isinstance(call_option['market_data'], dict):
                call_info.update(call_option['market_data'])

            # Add option greeks if available
            if 'option_greeks' in call_option and isinstance(call_option['option_greeks'], dict):
                call_info.update(call_option['option_greeks'])

            call_data.append(call_info)

        # Extract put option data if available
        if 'put_options' in item and isinstance(item['put_options'], dict):
            ...
            # Add market data if available
            if 'market_data' in put_option and isinstance(put_option['market_data'], dict):
                put_info.update(put_option['market_data'])

            # Add option greeks if available
            if 'option_greeks' in put_option and isinstance(put_option['option_greeks'], dict):
                put_info.update(put_option['option_greeks'])

            put_data.append(put_info)

    # Convert lists to DataFrames
    df_general = pd.DataFrame(general_data) if general_data else pd.DataFrame()
    df_call = pd.DataFrame(call_data) if call_data else pd.DataFrame()
    df_put = pd.DataFrame(put_data) if put_data else pd.DataFrame()
...
def extrcting_only_ltp_and_strikes(df_call):
  ...
  new_data_frame=df_call.iloc[:,[1,2]]
...
def calculate_all_possible_spread_and_max_loss(instrument_key,access_token_value):
  ...
  print(my_strike_list,new_data_frame)
  my_outer_list=[]
...
def max_pain_calculate(df_call,df_put):
  df_loss_corres_to_all_strikes=pd.DataFrame(columns=['strike_price','loss'])
  df_max_pain_cal=pd.DataFrame(data=[df_call['oi'],df_put['oi'],df_call['strike_price'],df_put['strike_price']]).transpose()
  df_max_pain_cal.columns=['oi_call','oi_put','strike_call','strike_put']
  for current_strike in df_max_pain_cal['strike_call']:



    df_max_pain_cal['loss_call_writers']=(df_max_pain_cal['strike_call']-current_strike)*df_max_pain_cal['oi_call']

    df_max_pain_cal['loss_put_writers']=(current_strike-df_max_pain_cal['strike_put'])*df_max_pain_cal['oi_put']


    max_pain=df_max_pain_cal[['loss_call_writers','loss_put_writers']].max(axis=1).sum()

    my_current_list=[current_strike,max_pain]
    df_loss_corres_to_all_strikes.loc[len(df_loss_corres_to_all_strikes)]=my_current_list

  value=df_loss_corres_to_all_strikes[df_loss_corres_to_all_strikes['loss']==df_loss_corres_to_all_strikes['loss'].min()]['strike_price']
  return df_loss_corres_to_all_strikes,value