...
import math
def linear_range(n,current_price,historical_stock_data):
...
def exponential_range(n,current_price,historical_stock_data):
  # data=yf.download(tickers=f'{symbol}.NS',period='max',interval='1d')
  # historical_stock_data=data['Adj Close']
  mean,std,req_data=cal_ploting_mean_std_returns(historical_stock_data)
...
def plotting_volatility_cone(no_days_before_expiry,historical_stock_data):


  mean,std,req_data=cal_ploting_mean_std_returns(historical_stock_data)

  n=no_days_before_expiry
...
def monte_carlo_simulation(current_price,historical_stock_data,no_of_days_to_simulate,num_simulations):
...
import numpy as np
import matplotlib.pyplot as plt
def geometric_brownian_motion_simulation(current_price,historical_stock_data,no_of_days_to_simulate,no_of_simulation_to_run):
# Define inputs
    mean,std,req_data=cal_ploting_mean_std_returns(historical_stock_data)
